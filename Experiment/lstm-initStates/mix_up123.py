...
if __name__ == "__main__":
    ...
    if True:
        ...
        fid = open('./result123_mixup/loss.txt','w')
        minLoss = 0.001
        ...
        for epoch in range(numEpoch):
            for index, (nn_in, nn_label) in enumerate(db.data_loader):
                ...
                # mix_up
                nn_in_train, label_train, lam, origin_train, perm_train = mixup_data(nn_in_train, nn_label, (device == 'cuda'))
            ...
            if epoch % 10 == 0:
                ...
                if (minLoss > loss_val_train):
                    torch.save({
                        ...
                        'loss': loss_train
                    }, './result123_mixup/model_best.tar')

        fid.close()
    elif (sys.argv[1] == 'eval'):
        if(torch.cuda.is_available()):
            checkpoint = torch.load('./result123_mixup/model_best.tar')
        else:
            checkpoint = torch.load('./result123_mixup/model_best.tar', map_location=torch.device('cpu'))
        ...
        fid = open("./result123_mixup/initStates_from_best.txt", 'w')
        ...
        Path("./result123_mixup/output").mkdir(parents=True, exist_ok=True)
        for index, (nn_in, nn_label) in enumerate(db.data_loader):
            ...
            for idxBatch in range(output.shape[1]):
                ...
                plt.draw()
                plt.savefig('./result123_mixup/output/predict_idxSeq_%d.png' % idxSeq)
                plt.close()
                fid = open("./result123_mixup/output/predict_idxSeq_%d.txt" % idxSeq, 'w')
